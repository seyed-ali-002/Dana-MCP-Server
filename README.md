...
The repository includes a visual snapshot of Dana from the local screenshots used during development:

![Dana screenshots overview](docs/images/dana-screenshots-overview.png)

The setup flow is also documented visually for each major client below.
...
![ChatGPT Developer Mode and Dana setup](docs/images/chatgpt-setup.svg)

1. Open **Settings → Apps → Advanced Settings** and enable **Developer Mode** when your account exposes that option.
2. Open **Apps → Create** (or the corresponding Workspace Apps → Create area for an admin).
3. Enter Dana's MCP endpoint and choose the required authentication method.
4. Click **Scan Tools**, complete authorization if prompted, then create the app.
5. In a chat, select the Dana app/connector for the message and verify that its tools are available.
...
![Dana plugin installation flow](docs/images/plugin-install.svg)
...
![Claude custom connector setup](docs/images/claude-setup.svg)
...
![Grok custom connector setup](docs/images/grok-setup.svg)
