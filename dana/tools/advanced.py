...
from dana.security.path_policy import require_output_path, require_path
...
def files(root: Path, suffixes: set[str] | None = None):
    ...
    for p in root.rglob("*"):
        if any(x in IGNORE for x in p.parts) or not p.is_file():
            continue
...
def run(cmd: list[str], cwd: str | None = None, timeout: int = 120):
    try:
        ...
        return {"returncode": -1, "stdout": "", "stderr": str(e), "command": cmd}
...
def register_advanced_tools(mcp: FastMCP) -> None:
    ...
    def database_schema(database: str) -> dict[str, Any]:
        ...
        db = require_path(database, purpose="database access")
    ...
    def container_logs(container: str, tail: int = 200) -> dict[str, Any]:
        if not shutil.which("docker"):
            return {"available": False, "reason": "docker not installed"}
        return run(
            ["docker", "logs", "--tail", str(max(1, min(tail, 5000))), container]
        )
    ...
    def docker_build(path: str = ".", tag: str = "dana-build:latest") -> dict[str, Any]:
        if not shutil.which("docker"):
            return {"available": False, "reason": "docker not installed"}
        return run(
            ["docker", "build", "-t", tag, str(Path(path).expanduser().resolve())],
            timeout=300,
        )
    ...
    def analyze_project(path: str = ".") -> dict[str, Any]:
        ...
        manifests = []
        for p in files(root):
            counts[p.suffix or "[no extension]"] = (
                counts.get(p.suffix or "[no extension]", 0) + 1
            )
            if p.name in {
                "pyproject.toml",
                "package.json",
                ...
                manifests.append(safe_rel(p, root))
        hints = []
    ...
    def find_entry_points(path: str = ".") -> dict[str, Any]:
        ...
        for p in files(root, {".py", ".js", ".ts", ".sh"}):
            ...
            if (
                ...
                or "FastAPI(" in t
                or "if __name__" in t
                or p.name in {"main.py", "app.py", "index.js", "server.js"}
            ):
                out.append(safe_rel(p, root))
    ...
    def project_health_check(path: str = ".") -> dict[str, Any]:
        ...
        if not (root / ".git").exists():
            issues.append("Git repository not detected")
        if not (root / "README.md").exists():
            issues.append("README.md missing")
        if not any(
            p.name
            in {"pyproject.toml", "package.json", "requirements.txt", "composer.json"}
            for p in root.iterdir()
        ):
            issues.append("Dependency manifest not detected")
    ...
    def find_duplicate_code(path: str = ".", min_lines: int = 6) -> dict[str, Any]:
        ...
        for p in files(root, {".py", ".js", ".ts", ".php"}):
            ...
            for i in range(max(0, len(lines) - min_lines + 1)):
                block = "\n".join(
                    x.strip() for x in lines[i : i + min_lines] if x.strip()
                )
                if len(block) < 40:
                    continue
                if block in seen:
                    dup.append(
                        {
                            "first": seen[block],
                            "duplicate": safe_rel(p, root) + f":{i + 1}",
                        }
                    )
                else:
                    seen[block] = safe_rel(p, root) + f":{i + 1}"
        return {"duplicates": dup[:200], "count": len(dup)}

    @mcp.tool()
    def analyze_stacktrace(text: str) -> dict[str, Any]:
        lines = text.splitlines()
        frames = [x.strip() for x in lines if "File " in x or re.search(r"\bat\s+", x)]
        errors = [
            x.strip()
            for x in lines
            if re.search(r"(Error|Exception|Traceback|FAILED)", x)
        ]
        return {"frames": frames, "errors": errors[-10:], "raw_tail": lines[-30:]}

    @mcp.tool()
    def tail_logs(path: str, lines: int = 100) -> dict[str, Any]:
        p = require_path(path, purpose="log access")
        data = p.read_text(errors="ignore").splitlines()[-max(1, min(lines, 5000)) :]
    ...
    def search_logs(path: str, pattern: str, limit: int = 200) -> dict[str, Any]:
        ...
        matches = [
            {"line": i + 1, "text": x}
            for i, x in enumerate(p.read_text(errors="ignore").splitlines())
            if rx.search(x)
        ]
    ...
    def secret_scan(path: str = ".") -> dict[str, Any]:
        ...
        patterns = {
            ...
            "generic_secret": r'(?i)(api[_-]?key|secret|token|password)\s*[:=]\s*["\']?[^\s"\']{8,}',
        }
        hits = []
        for p in files(root):
    ...
    def dependency_security_scan(path: str = ".") -> dict[str, Any]:
        ...
        if (root / "pyproject.toml").exists() or (root / "requirements.txt").exists():
            ...
            if audit:
                results["python"] = run([audit], str(root))
            else:
                results["python"] = {
                    "skipped": True,
                    "reason": "pip-audit not installed; install Dana security extra",
                }
        if not results:
            return {
                "skipped": True,
                "reason": "No supported dependency manifest detected",
            }
    ...
    def dependency_outdated(path: str = ".") -> dict[str, Any]:
        root = require_path(path, purpose="project access")
        if (root / "package.json").exists() and shutil.which("npm"):
            return run(["npm", "outdated", "--json"], str(root))
    ...
    def find_symbol(path: str, symbol: str) -> dict[str, Any]:
        ...
        found = []
        rx = re.compile(
            r"\b(?:def|class|function|const|let|var)\s+" + re.escape(symbol) + r"\b"
        )
        for p in files(root, {".py", ".js", ".ts", ".tsx", ".jsx"}):
            for i, l in enumerate(p.read_text(errors="ignore").splitlines(), 1):
                if rx.search(l):
                    found.append(
                        {"file": safe_rel(p, root), "line": i, "text": l.strip()}
                    )
