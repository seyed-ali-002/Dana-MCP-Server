...
def register_web_quality_debug_docs_tools(mcp: FastMCP) -> None:
    ...
    def web_fetch(url: str, headers: dict[str, str] | None = None) -> dict[str, Any]:
        ...
        with urllib.request.urlopen(req, timeout=30) as r:
            return {
                ...
                "body": r.read(500000).decode(errors="replace"),
            }
    ...
    @mcp.tool()
    def benchmark(
        command: list[str], cwd: str | None = None, runs: int = 3
    ) -> dict[str, Any]:
        out = []
    ...
    def generate_changelog(path: str = ".") -> str:
        r = _run(
            ["git", "log", "--pretty=format:- %h %s", "-30"], str(Path(path).resolve())
        )
        return (
            "# Changelog\n\n## Recent Changes\n"
    ...
    def architecture_summary(path: str = ".") -> dict[str, Any]:
        ...
        for f in root.rglob("*"):
            if f.is_file() and not any(
                x in f.parts for x in (".git", ".venv", "node_modules")
            ):
    ...
    def generate_project_diagram(path: str = ".") -> str:
        ...
        for item in sorted(root.iterdir()):
            ...
            lines.append(f'ROOT --> {safe}["{item.name}"]')
        return "\n".join(lines)

    @mcp.tool()
    def generate_report(
        title: str, data: dict[str, Any], output: str | None = None
    ) -> str:
        report = f"# {title}\n\n```json\n{json.dumps(data, indent=2, ensure_ascii=False)}\n```\n"
        if output:
            Path(output).expanduser().resolve().write_text(report)
        return report
