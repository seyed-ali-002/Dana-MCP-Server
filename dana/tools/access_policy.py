...
from dana.security.path_policy import policy_status, set_allowed_paths, add_allowed_path, remove_allowed_path, is_allowed, require_path
...
def register_access_policy_tools(mcp: FastMCP) -> None:
 ...
 def validate_path_access(path: str) -> dict:
  ...
  try: target=require_path(path); return {"path":str(target),"allowed":True}
  except PermissionError as e: return {"path":path,"allowed":False,"reason":str(e)}
